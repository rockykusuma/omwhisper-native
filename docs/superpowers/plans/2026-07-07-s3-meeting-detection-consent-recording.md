...
> **Amendment (2026-07-08, after live verification):** Task 3 and Task 6's `MeetingRecorder` code below still describes the originally-planned ScreenCaptureKit approach — kept as-is for historical accuracy of what was planned/executed at the time, not updated in place. It was replaced post-implementation: SCK's audio-only capture still triggers the full Screen Recording permission and a system-wide "Currently Sharing" indicator (confirmed live, and correctly flagged by the user as disproportionate for an audio-only feature). The shipped `MeetingRecorder.swift` instead uses CoreAudio process taps + an aggregate device, read via raw HAL I/O (`AudioDeviceCreateIOProcIDWithBlock`/`AudioDeviceStart`, not `AVAudioEngine` — an `AVAudioEngine`-based attempt never worked live) — see the design spec's matching amendment and the file's own header comment for the full rationale, including the `NSAudioCaptureUsageDescription` Info.plist key this needs (a separate TCC gate from `NSMicrophoneUsageDescription`, silently denied without it — no prompt, no error). Net effect: only the "System Audio Recording Only" permission tier, no Screen Recording prompt at all. Live-verified via the debug self-test: both tracks captured correctly (system audio -8.7dBFS, mic -17.8dBFS).
